...
private:
	...
	void Generate_bidiagonal(T min_dist, T max_dist, int rand_num)
	{
		...
		SquareMatrix SVD_matr = SVD.MatrixU() * true_sigm_B * SVD.MatrixV().transpose();
		auto bid = Eigen::internal::UpperBidiagonalization<SquareMatrix>(SVD_matr);  //Бидиагонализация
		B = bid.bidiagonal();
	...
	void Impl_QR_zero_iter()
	{
		...
		for (int i = 0; i != n - 1; i++)
		{
			...
			cs = temp1[0]; sn = temp1[1]; r = temp1[2];

			//(cs, -sn) 
			//(sn, cs)
			SquareMatrix Temp_J_R = SquareMatrix::Identity(n, n);
			Temp_J_R(i, i) = cs; Temp_J_R(i, i+1) = -sn;
			Temp_J_R(i+1, i) = sn; Temp_J_R(i+1, i+1) = cs;
	...
	//Переносим знак у сингулярного значения на соответсвующий вектор в левой матрице. (ЛОМАЕТСЯ ОРТОГОНАЛЬНОСТЬ!)
	...
	//Нужна для работы функции "shifts", Подсчёт нормы ||B-B*||, где B - изначальная матрица, B* - матрица составленная перемножением поворотов. 
	T differance_norm(Eigen::VectorXd sines, Eigen::VectorXd cosines, bool true_sigm = false) //Булево значение нужно для понимания того, из каких сингулярных значений составляется B* (настоящих или посчитанных) 
	{
		T cs; T sn;
		...
		for (int i = 0; i != (n - 1) * iter_num; i++)
		{
			...
			cs = cosines(2 * i); sn = sines(2 * i);
			SquareMatrix Temp_J_R = SquareMatrix::Identity(n, n);
			Temp_J_R(i % (n - 1), i % (n - 1)) = cs; Temp_J_R(i % (n - 1), i % (n - 1) + 1) = -sn;
			Temp_J_R(i % (n - 1) + 1, i % (n - 1)) = sn; Temp_J_R(i % (n - 1) + 1, i % (n - 1) + 1) = cs;

			temp_right_J = temp_right_J * Temp_J_R;

			//Составляем матрицу поворота слева
			cs = cosines(2 * i + 1); sn = sines(2 * i + 1);
			SquareMatrix Temp_J_L = SquareMatrix::Identity(n, n);
			Temp_J_L(i % (n - 1), i % (n - 1)) = cs; Temp_J_L(i % (n - 1), i % (n - 1) + 1) = sn;
			Temp_J_L(i % (n - 1) + 1, i % (n - 1)) = -sn; Temp_J_L(i % (n - 1) + 1, i % (n - 1) + 1) = cs;
		...
		if (true_sigm)
		{
			//Для настоящих значений нужно поменять знак на соответсвующий при том же посчитанном (ненастоящим) сингулярном значпении 
			SquareMatrix temp_true_sigm = true_sigm_B;
			for (int i = 0; i != n; i++)
			{
				if (sigm_B(i, i) < 0)
					temp_true_sigm(i, i) = -temp_true_sigm(i, i);
			}
			temp_B = temp_left_J.transpose() * temp_true_sigm * temp_right_J.transpose(); //Составляем B*

		}
		else
		{
			SquareMatrix temp_sigm = sigm_B;
			temp_B = temp_left_J.transpose() * sigm_B * temp_right_J.transpose(); //Составляем B*
		}
		return((B - temp_B).norm());
	}

public:
	QR_zero_shift(int n1, T min_dist, T max_dist, int rand_num) //Конструктор использующий генератор бидиагональных матриц
	{
		assert(n1 > 1);
	...
	QR_zero_shift(SquareMatrix b) //Конструктор с задачей матрицы B, использующий СВД из Eigen
	{
		n = b.rows();
		iter_num = 0;
	...
	//Основная функция задающая количество итераций алгоритма."null_superdiagonal" - , "null_superdiagonal" - зануление супердиагонали полученный матрицы с сингулярными значениями
	void Implicit_QR_with_zero_shift(int m, bool null_superdiagonal = false, bool revert_negative = false) //m - количество итераций алгоритма
	{
	...
	//Подсчёт нормы || B - B* || , где B - изначальная матрица, B* - матрица составленная перемножением поворотов.
	T differance_norm(bool true_sigm = false)
	{
		...
		for (int i = 0; i != (n - 1) * iter_num; i++)
		{
			...
			cs = NewCosines(2 * i); sn = NewSines(2 * i);
			SquareMatrix Temp_J_R = SquareMatrix::Identity(n, n);
			Temp_J_R(i % (n - 1), i % (n - 1)) = cs; Temp_J_R(i % (n - 1), i % (n - 1) + 1) = -sn;
			Temp_J_R(i % (n - 1) + 1, i % (n - 1)) = sn; Temp_J_R(i % (n - 1) + 1, i % (n - 1) + 1) = cs;
			...
			cs = NewCosines(2 * i + 1); sn = NewSines(2 * i + 1);
			SquareMatrix Temp_J_L = SquareMatrix::Identity(n, n);
			Temp_J_L(i % (n - 1), i % (n - 1)) = cs; Temp_J_L(i % (n - 1), i % (n - 1) + 1) = sn;
			Temp_J_L(i % (n - 1) + 1, i % (n - 1)) = -sn; Temp_J_L(i % (n - 1) + 1, i % (n - 1) + 1) = cs;
		...
		if (true_sigm)
		{
			...
			temp_B = temp_left_J.transpose() * temp_true_sigm * temp_right_J.transpose();
	...
	//Идея сдвигов: для каждого угла, для каждого поворота изменяем значение синуса на сдвиг, изменяем соответсвенно косинус и пересчитываем норму, если норма меньше, то сохраняем новое значение
	T shifts(Eigen::VectorXd shifts, bool true_sigm = false)
	{
		...
		for(int i = 0; i != trigonom_i; i++)
			for (T shift : shifts)
			{
				...
				temp_new_cosines(i) = (NewCosines(i) < 0) ? -sqrt(1 - pow(temp_new_sines(i), 2)) : sqrt(1 - pow(temp_new_sines(i), 2));
	...
	//Строим матрицу слева перемножением поворотов с новыми значениями синусов и косинусов
	...
	//Строим матрицу справа перемножением поворотов с новыми значениями синусов и косинусов
