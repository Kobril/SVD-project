...
//В файле generate_svd.cpp вызывается конструктор SVDGenerator из файла generate_svd.h для создания матриц U,V,SIGMA. 
//Выводится произведение матриц U и транспонированной U.
//Функция generate из файла generate_svd.h  используется для заполнения матрицы SIGMA и получения матриц U,V. 
//В нее передается количество  сингулярных значений.
//В файле generate_svd.h описан класс и все необходимые параметры и функции.
//В файле generate_svd.cpp создаются и заполняются матрицы U,V,SIGMA посредством обращения к необходимым методам и параметрам  в generate_svd.h 
//Конструктор создает необходимые матрицы. В него предаются параметры: число строк, число столбцов, генератор случайных чисел с параметром, заданное распределение и флаг необходимости включения в сингулярные значения границ распределения.
